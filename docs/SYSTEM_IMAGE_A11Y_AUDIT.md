...
- Current count: 212 `Image(systemName:)` calls.
...
| Icon + visible text controls | 56 | wallet rows, editor/upload actions, settings rows | Visible text is the accessible label |
| Standalone toolbar/icon buttons | 36 | share, notification, settings, support, search back, market create, camera controls, modal close controls | Parent control carries explicit label or stable action ID; manual VoiceOver pass required |
| Status/metadata icons | 40 | rating stars, verified badges, wallet ledger icons, rejection policy icons, progress checkmarks | Parent row/card text owns state, or row is combined |
| Decorative/illustrative icons | 29 | placeholders, empty states, preview ornaments, PhotoImport permission notices, upload thumbnail header icon | Decorative; hidden or contained by parent surface |
