...
> 본 문서는 filterMarket 의 모든 모션·전환·햅틱 동작의 단일 진실원이다.
...
filterMarket 의 모션은 **세 단어**로 요약된다: **절제, 명료, 즉각**.
...
필터 스와이프는 filterMarket 의 **핵심 인터랙션**이다. 사용자가 카메라 라이브뷰 위에서 좌우로 스와이프해 실시간으로 필터를 전환한다.
