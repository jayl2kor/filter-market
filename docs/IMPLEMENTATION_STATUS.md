...
> 마지막 업데이트: 2026-05-06 17:30 KST · 기준 커밋/브랜치: 로컬 작업 상태 · 상태: Phase 0 실기기 검증 대기 / 필터 적용본 PhotoKit 저장 경로 구현 완료
>
...
| `Camera` | [CameraSession.swift](../Sources/Camera/CameraSession.swift), [PhotoLibrarySaver.swift](../Sources/Camera/PhotoLibrarySaver.swift) | 권한 요청, back camera session, video frame callback, `AVCapturePhotoOutput` photo data capture, PhotoKit add-only 저장 service |
...
실기기 검증과 병행 가능한 다음 코드 작업은 **전/후면 전환, 탭 포커스/노출, 비율 전환**이다. 셔터는 이제 `AVCapturePhotoOutput`으로 원본 photo data를 확보하고, 저장 전 이미지에 현재 `FilterRenderConfiguration`을 적용한 뒤 PhotoKit 저장 경로까지 연결한다.
...
1. 실기기에서 촬영 원본/필터링 결과 및 PhotoKit 저장 검증
2. 전/후면 전환, 탭 포커스/노출, 비율 전환 추가
