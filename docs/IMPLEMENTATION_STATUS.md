...
> 마지막 업데이트: 2026-05-06 17:17 KST · 기준 커밋/브랜치: 로컬 작업 상태 · 상태: Phase 0 실기기 검증 대기 / 고해상 촬영 결과 필터 적용 경로 구현 완료
>
...
- 실기기 카메라 preview 검증
- 실기기 FPS/GPU time 측정
- 실기기 촬영 결과 필터 적용 검증 및 PhotoKit 저장
...
| `Camera` | [CameraSession.swift](../Sources/Camera/CameraSession.swift) | 권한 요청, back camera session, video frame callback, `AVCapturePhotoOutput` photo data capture |
...
| M1-A05 | PhotoKit 저장 구현 | Not Started | photo data 획득 완료, 저장 권한/PhotoKit write 후속 |
...
실기기 검증과 병행 가능한 다음 코드 작업은 **필터링된 촬영 결과의 PhotoKit 저장 및 권한 처리**다. 셔터는 이제 `AVCapturePhotoOutput`으로 원본 photo data를 확보하고, 저장 전 이미지에 현재 `FilterRenderConfiguration`을 적용한다.
...
1. PhotoKit 저장 및 권한 처리
2. 실기기에서 촬영 원본/필터링 결과 시각 검증
3. 전/후면 전환, 탭 포커스/노출, 비율 전환 추가
4. 핵심 플로우 테스트 및 실기기 smoke test
