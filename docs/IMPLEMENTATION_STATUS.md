...
- `AVCapturePhotoOutput` 기반 셔터 촬영과 captured photo data result sheet 연결
...
- 1:1, 4:3, 16:9 촬영 비율 선택 UI와 저장 전 center crop 적용
...
- `FilterEngineTests.PhotoFilterRendererTests`: 6개 통과
- `MarketplaceTests.BundleSeedFilterRepositoryTests`: 4개 통과
- 총 36개 테스트, 0 failures
...
| M1-A03 | 촬영 비율 1:1, 4:3, 16:9 전환 | Partial | segmented ratio picker, preview guide, 저장 전 center crop 완료. 실기기 guide/저장 결과 일치 검증 필요 |
...
| M1-B03 | 고해상 이미지에 동일 필터 chain 적용 | Partial | CPU `PhotoFilterRenderer`와 capture result 연결 완료. 실기기 촬영 결과 시각 검증 필요 |
...
다음 우선 작업은 **실기기 Camera MVP smoke test**다. 셔터는 이제 `AVCapturePhotoOutput`으로 원본 photo data를 확보하고, 저장 전 이미지에 현재 `FilterRenderConfiguration`과 선택 비율 crop을 적용한 뒤 PhotoKit 저장 경로까지 연결한다. 전/후면 전환, 탭 포커스/노출, 비율 전환은 코드 경로와 simulator build/test를 통과했으므로 실기기 preview/촬영 방향, AF/AE, guide/저장 결과 일치 검증이 남았다.

### 완료된 진행: M1-A03 촬영 비율 전환

목표: 사용자가 1:1, 4:3, 16:9 중 촬영 비율을 선택하고, preview guide와 저장 JPEG의 center crop 비율이 일치하도록 한다.

| 순서 | 작업 | 상태 |
|---|---|---|
| 1 | `PhotoCropAspectRatio` 모델 추가 | Done |
| 2 | 카메라 화면 segmented ratio picker 추가 | Done |
| 3 | preview aspect guide overlay 추가 | Done |
| 4 | 저장 전 필터 적용 JPEG center crop 연결 | Done |
| 5 | portrait/landscape 비율 해석 및 crop 테스트 추가 | Done |
| 6 | 문서 갱신 | Done |

완료 조건:
- [x] 카메라 화면에서 1:1, 4:3, 16:9 비율을 선택할 수 있다.
- [x] portrait preview에서 4:3/16:9는 각각 3:4/9:16 guide로 표시된다.
- [x] 저장 전 filtered JPEG가 선택 비율로 center crop된다.
- [x] capture result summary가 선택 비율을 표시한다.
- [x] Simulator unit test가 square crop과 portrait aspect 해석을 검증한다.
- [ ] 실기기에서 guide와 저장 결과 비율 일치를 확인한다.
...
목표: 카메라 화면의 flip 버튼이 active `AVCaptureDeviceInput`을 전/후면으로 교체하고, preview/photo connection 방향과 미러링 상태를 전환한다.

| 순서 | 작업 | 상태 |
|---|---|---|
| 1 | `CameraPosition` 모델과 toggle 로직 추가 | Done |
| 2 | `CameraSession` input 교체 경로 추가 | Done |
| 3 | preview/photo connection 회전과 front mirroring 갱신 | Done |
| 4 | SwiftUI flip 버튼과 switching state 연결 | Done |
| 5 | 단위 테스트 추가 | Done |
| 6 | 문서 갱신 | Done |

완료 조건:
- [x] flip 버튼은 촬영 중/전환 중 중복 실행되지 않는다.
- [x] session configuration 안에서 기존 input 제거와 새 input 추가를 수행한다.
- [x] 전면 카메라는 가능한 connection에서 미러링을 적용한다.
- [x] Simulator unit test가 position toggle 로직을 검증한다.
- [ ] 실기기에서 전/후면 preview 방향과 촬영 결과 방향을 확인한다.

### 완료된 진행: M1-A05 PhotoKit 저장

목표: capture result sheet의 `Save` 버튼이 필터 적용본 JPEG를 사용자 사진 라이브러리에 저장하고, 권한/성공/실패 상태를 명확히 표시한다.

| 순서 | 작업 | 상태 |
|---|---|---|
| 1 | `PhotoLibrarySaving` 추상화 추가 | Done |
| 2 | add-only 권한 요청 구현 | Done |
| 3 | 필터링된 JPEG 저장 연결 | Done |
| 4 | capture result sheet 상태 UI 추가 | Done |
| 5 | 테스트 추가 | Done |
| 6 | 문서 갱신 | Done |

완료 조건:
- [x] `Save` 버튼은 원본이 아니라 필터 적용본을 저장한다.
- [x] 저장 성공 후 사용자가 결과를 인지할 수 있다.
- [x] 권한 거부 상태에서 앱이 실패하지 않고 안내 상태를 표시한다.
- [x] Simulator unit test가 PhotoKit service 경계를 검증한다.
- [ ] 실기기에서 Photos 권한 요청과 저장 결과를 확인한다.
...
| P1 | 비율 1:1/4:3/16:9 | 코드 경로 완료, 실기기 검증 필요 |
...
1. 실기기에서 preview, 촬영 원본/필터링 결과, PhotoKit 저장, 전/후면 전환 검증
2. 실기기에서 탭 포커스/노출, 비율 guide/저장 결과 일치 검증
3. 검증 결과 기반 preview orientation/crop 보정 또는 4-pass 파이프라인 정리
