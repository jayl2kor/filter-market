# filterMarket - Design Integration Plan
...
| D3 | 🟡 Partial (6/8) | 2026-05-06 | — | Batch 1 완료: `LoginScreen` (02), `MarketplaceScreen` (06), `FilterDetailScreen` (07). Batch 2 완료: `ProfileScreen` (09) — `Sources/App/Profile/` 신설, 헤더 + 통계 + 세그먼트 + 3열 정사각 그리드 + 톱니바퀴→`SettingsScreen` 푸시; `SettingsScreen` (10) — `Sources/App/Settings/` 신설, 그룹 리스트 (계정·카메라·알림·정보·로그아웃·계정삭제) + 토글/picker/destructive 알림; `SearchScreen` (08) — `Sources/App/Search/` 신설, browsing/typing/results 3-phase + 최근 검색 + 추천 키워드 칩 + 인기 메이커 가로 스크롤 + 결과 그리드 + `noSearchResults` 빈 상태. `ProfileMockData` 추가. 이전 `Sources/App/ProfileScreen.swift` / `Sources/App/SearchScreen.swift` placeholder 제거. 남은 화면: 01 Onboarding / 05 Capture Preview. |
| D4 | ⏳ Pending | — | — | — |
| D5 | ⏳ Pending | — | — | — |
| D6 | ⏳ Pending | — | — | — |
| D7 | ⏳ Pending | — | — | MVP 외 |
