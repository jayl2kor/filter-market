...
| **내부 화폐 (Coin) 충전 + 사용** | Must | Phase 6 |
| Pro 멤버십 (모든 유료 필터 무제한) | Should | Phase 6 |
| 유료 필터 / 메이커 코인 적립 | Could | Phase 6 |
...
US-06 [메이커] 내 필터에 가격(예: 50 코인)을 매기고, 사용자가 구매할 때마다 60%를 코인으로 적립받고 싶다 (Phase 6).
...
| 수익 (Phase 6) | 코인 충전 매출 (Apple IAP 통과 후 net) | - | \$50K/월 |
| 수익 (Phase 6) | Pro 멤버십 MRR | - | \$30K/월 |
| 수익 (Phase 6) | 메이커 적립 코인 / 사용 코인 비율 (코인 속도) | - | 0.6 ± 0.1 |
...
> 결제 모델 단일 진실원: [`CURRENCY_DESIGN.md`](./CURRENCY_DESIGN.md)

- **내부 화폐 "Coin"** — 사용자는 Apple IAP로 코인 패키지(100/550/1,200/3,000)를 충전하고 코인으로 필터를 구매. 직접 통화 결제 없음(IAP 가이드 3.1.1 준수)
- **메이커 분배 60%** — 필터 1건 판매 시 메이커가 코인으로 60% 적립, moodit 운영비 40%
- **Pro 멤버십** — 별도 트랙. 월 ₩4,900 / 연 ₩34,800 — 모든 유료 필터 무제한 + 월 300 코인 자동 적립
- **메이커 출금** — Stripe Connect 통해 적립 코인 → 원화 환전 (1 코인 = ₩14, 임계 5,000 코인). KYC + 세무 정보 필수
- 영상 / Reels-style / AR 얼굴 필터(Vision/ARKit) — 코인 모델 안에서 동일 구매
...
- 결제(Phase 6): 코인 충전은 **Apple IAP 단독** (수수료 30%/소상공인 15%) → moodit 내부 거래는 추가 수수료 없음. 메이커 출금만 Stripe Connect 사용
...
- [CURRENCY_DESIGN.md](./CURRENCY_DESIGN.md) — Coin 화폐 모델 (분배·패키지·출금·컴플라이언스)
