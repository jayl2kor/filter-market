...
| Screen ID | Route / Entry | SwiftUI | 상태 | Required actions |
|---|---|---|---|---|
| `camera.live` | `tab.shutter`, UI test route | `CameraScreen` | Partial, Blocked on real device | `camera.dismiss`, `camera.flip`, `camera.aspectRatio`, `camera.timer`, `camera.grid.toggle`, `camera.flash`, `camera.filterIntensity`, `camera.filterSwipe`, `camera.filter.*`, `camera.openLibrary`, `camera.shutter`, `camera.shutter.longPress`, `camera.zoom.*` |
